...
/*
 * Class:     com_android_usbtuner_TunerHalSatellite
 * Method:    nativeTune
 * Signature: (JILjava/lang/String;)Z
 */
JNIEXPORT jboolean JNICALL
Java_com_android_usbtuner_TunerHal_nativeTuneSatellite
(JNIEnv *env, jobject thiz, jlong deviceId, jint frequency, jstring modulation, jint timeout_ms, jstring polarization, jint symbolrate, jstring fec) {
    std::map<jlong, DvbManager *>::iterator it = sDvbManagers.find(deviceId);
    DvbManager *dvbManager;
    if (it == sDvbManagers.end()) {
        dvbManager = new DvbManager(env, thiz);
    ...
    int res = dvbManager->tuneSatellite(env, thiz,
            frequency, env->GetStringUTFChars(modulation, 0), timeout_ms, env->GetStringUTFChars(polarization, 0), symbolrate, env->GetStringUTFChars(fec, 0));
    return (res == 0);
}
