...
public abstract class TunerHal implements AutoCloseable {
    ...
    @StringDef({ MODULATION_8VSB, MODULATION_QAM256, MODULATION_QPSK })
    @Retention(RetentionPolicy.SOURCE)
    ...
    protected static final int PID_CAT = 0x01;
    protected static final int PID_ATSC_SI_BASE = 0x1ffb;
    protected static final int PID_SDT = 0x0011;
    ...
    public boolean tuneSatellite(int frequency, @ModulationType String modulation, String polarization, int symbolrate, String fec) {
        Log.i(TAG, "tuneSatellite");
        if (!isDeviceOpen()) {
        ...
        if (nativeTuneSatellite(getDeviceId(), frequency, modulation, timeout_ms, polarization, symbolrate, fec)) {
            addPidFilter(PID_PAT, FILTER_TYPE_OTHER);
            addPidFilter(PID_SDT, FILTER_TYPE_OTHER);
            addPidFilter(PID_CAT, FILTER_TYPE_OTHER);
    ...
    protected native boolean nativeTuneSatellite(long deviceId, int frequency,
            @ModulationType String modulation, int timeout_ms, String polarization, int symbolrate, String fec);
