...
int DvbManager::tuneSatellite(JNIEnv *env, jobject thiz,
        const int frequency, const char *modulationStr, int timeout_ms, const char *polarizationStr, int symbolrate, const char *fecStr) {
    ...
    ALOGD("DvbManager::tuneSatellite");
    resetExceptFe();


    struct dvb_frontend_parameters feParams;
    memset(&feParams, 0, sizeof(struct dvb_frontend_parameters));
    polarization = toupper(polarizationStr[0]);
	if (frequency > 2200000) {
        if (frequency < SLOF) {
            feParams.frequency=(frequency-LOF1);
	        hi_lo = 0;
	        base = LOF1;
        } else {
            feParams.frequency=(frequency-LOF2);
	        hi_lo = 1;
	        base = LOF2;
        }
    } else {
    ...
    polv = (polarization == 'V' ? 1 : 0);
    if(ioctl(mFeFd, FE_SET_VOLTAGE, (polv ? SEC_VOLTAGE_13 : SEC_VOLTAGE_18)) < 0) {
        ALOGE("FE_SET_VOLTAGE failed %s", polarization);
        return -1;
    }
   	if(ioctl(mFeFd, FE_SET_TONE, (hi_lo ? SEC_TONE_ON : SEC_TONE_OFF)) < 0) {
        ALOGE("FE_SET_TONE failed %d", hi_lo);
        return -1;
    }
...
int DvbManager::startTsPidFilter(JNIEnv *env, jobject thiz, int pid, int filterType) {
    ...
    filter.pid = pid;
    ALOGD("startTsPidFilter - pid=%d", pid);
