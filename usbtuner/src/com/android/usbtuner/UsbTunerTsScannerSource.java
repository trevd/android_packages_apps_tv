...
public class UsbTunerTsScannerSource implements InputStreamSource {
    ...
    public void startStream() {
        ...
        Log.i(TAG, "Streaming started mBytesFetched=" + mBytesFetched);
    }

    @Override
    public boolean setScanChannel(ScanChannel channel) {
        Log.i(TAG, "setScanChannel - Attempting Tune - .deliverysystem:" + channel.deliverysystem);
        switch (channel.deliverysystem) {
        ...
        case DELIVERY_SYSTEM_DVBS:
            if (mTunerHal.tuneSatellite(channel.frequency, channel.modulation, channel.polarization, channel.symbolrate, channel.fec)) {
                Log.i(TAG, "tuneSatellite True");
                mEventDetector.startDetecting(channel.frequency, channel.modulation);
                mDeviceConfigured = true;
                return true;
            }
